...
- **Input Pane** (Top): Where you compose HTTP requests
- **Output Pane** (Bottom): Displays HTTP responses
...
- **Ctrl+W h**: Move to Input pane
- **Ctrl+W l**: Move to Output pane
...
- **Ctrl+K**: Move boundary upward (shrink Input / expand Output)
- **Ctrl+J**: Move boundary downward (expand Input / shrink Output) 
...
### Request Format
Compose HTTP requests in the Input pane using this format:
...
### Response Display
Responses appear in the Output pane with:
...
- Network errors displayed in Output pane
...
| Command | Action |
|---------|--------|
| `:q` | Quit application (or hide Output pane if in Response pane) |
| `:quit` | Same as `:q` |
